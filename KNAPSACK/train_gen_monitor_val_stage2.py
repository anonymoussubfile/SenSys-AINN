...
def gen(model_ori, train_model, dataset, rng_key, mode = 'train'):
    ...
    #return
    with open('monitor/_pinf_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_pinf_list))

    with open('monitor/_weight_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_w_list))
    with open('monitor/_profit_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_v_list))
    with open('monitor/_pred_ratio_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_p_list))
    with open('monitor/_gt_ratio_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_gt_list))
    with open('monitor/_inf_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_inf_list))


    with open('monitor/weight_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(w_list))
    with open('monitor/profit_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(v_list))
    with open('monitor/pred_ratio_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(p_list))
    with open('monitor/gt_ratio_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(gt_list))
    with open('monitor/inf_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(inf_list))
    with open('monitor/pinf_list5_log_exp_{}_stage2.npy'.format(mode), 'wb') as f:
