...
import copy
from run_eval_outputs import main, run_inference
best_acc = 0
...
def gen(model_ori, train_model, model2, dataset, rng_key, mode = 'train'):
    ...
    with open('monitor/_weight_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_w_list))
    with open('monitor/_profit_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_v_list))
    with open('monitor/_pred_ratio_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_p_list))
    with open('monitor/_k_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_k_list))
    with open('monitor/_c_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_c_list))
    with open('monitor/_kl_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(_kl_list))

    with open('monitor/weight_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(w_list))
    with open('monitor/profit_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(v_list))
    with open('monitor/pred_ratio_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(p_list))
    with open('monitor/k_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(k_list))
    with open('monitor/c_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(c_list))
    with open('monitor/kl_list5_log_exp_{}_stage3.npy'.format(mode), 'wb') as f:
        np.save(f, np.array(kl_list))

model_ori = SimpleNN()
model_ori = model_ori.to('cuda')
state_dict = torch.load(
        'model_ainn_stage1_{}.pth'.format(num_sample), map_location='cuda'
    )
...
state_dict = torch.load(
        'model_ainn_stage3_{}.pth'.format(num_sample), map_location='cuda'
    )
