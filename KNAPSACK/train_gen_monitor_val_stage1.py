...
def gen(model_ori, dataset, train_mode = 'train'):
    ...
    with open('monitor/label_list5_log_exp_{}_stage1.npy'.format(train_mode), 'wb') as f:
        np.save(f, np.array(label_list))

    with open('monitor/gt_ratio_list5_log_exp_{}_stage1.npy'.format(train_mode), 'wb') as f:
        np.save(f, np.array(gt_ratio_list))

    with open('monitor/pred_ratio_list5_log_exp_{}_stage1.npy'.format(train_mode), 'wb') as f:
